<?php
    ...
    $orario=$_GET["orario"];
    $prezzo=$_GET["prezzo"];
    $distanzaPercorsa=$_GET["distanzaPercorsa"];
    ...
    $sql= "INSERT INTO operazione (tipoOperazione, orario, prezzo, distanzaPercorsa, IDCliente, IDBici, IDStazione) VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    //ss --> 2 stringhe
    $stmt->bind_param("ssddiii", $tipoOperazione, $orario, $prezzo, $distanzaPercorsa, $idCliente, $idBici, $idStazione);
    
    //esecuzione della query creata
    if ($stmt->execute()){
        //salvataggio della risposta in un nuovo array
        $arr = array("status" => "ok", "message" => "Operazione inserita");
